# Instruções de Deploy SSL na VPS

## Problema Atual
Os três domínios abaixo estão com problemas de certificado SSL:
- https://api-aponta.pedroct.com.br/health
- https://mcp.treit.com.br/health
- https://ds.treit.com.br

## Solução
Configurar um container nginx como reverse proxy com os certificados SSL do Cloudflare.

## Passo 1: Conectar na VPS

```bash
ssh root@srv1264175.hstgr.cloud
```

## Passo 2: Verificar containers em execução

```bash
docker ps
```

Você deve ver os containers:
- `treit-design-system-ds-site-1` (porta 3000)
- `treit-design-system-mcp-1` (porta 8787)
- `api-aponta` (porta 8000)

## Passo 3: Criar diretório para nginx

```bash
mkdir -p /opt/nginx-ssl/{conf,ssl}
cd /opt/nginx-ssl
```

## Passo 4: Criar os certificados SSL

### Certificado ds.treit.com.br

```bash
cat > /opt/nginx-ssl/ssl/fullchain-ds.pem <<'EOF'
-----BEGIN CERTIFICATE-----

E1pPCOwl3SehjMThJzsWBJHx4ek4voN6tOUG
-----END CERTIFICATE-----
EOF

cat > /opt/nginx-ssl/ssl/privkey-ds.pem <<'EOF'
-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----
EOF
```

### Certificado mcp.treit.com.br

```bash
cat > /opt/nginx-ssl/ssl/fullchain-mcp.pem <<'EOF'
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
EOF

cat > /opt/nginx-ssl/ssl/privkey-mcp.pem <<'EOF'
-----BEGIN PRIVATE KEY-----

ug6Hy0a3pzl/kG+IagETR+3B
-----END PRIVATE KEY-----
EOF
```

### Certificado api-aponta.pedroct.com.br

**NOTA:** Você precisa gerar este certificado no Cloudflare para o domínio `pedroct.com.br` ou `api-aponta.pedroct.com.br`.

Por enquanto, vamos criar links simbólicos apontando para o certificado do mcp (temporário):

```bash
ln -s /opt/nginx-ssl/ssl/fullchain-mcp.pem /opt/nginx-ssl/ssl/fullchain-api.pem
ln -s /opt/nginx-ssl/ssl/privkey-mcp.pem /opt/nginx-ssl/ssl/privkey-api.pem
```

**IMPORTANTE:** Substitua pelos certificados corretos assim que gerá-los no Cloudflare.

## Passo 5: Criar configuração do nginx

```bash
cat > /opt/nginx-ssl/conf/nginx.conf <<'EOF'
events {
    worker_connections 1024;
}

http {
    # Upstream para api-aponta (já existente)
    upstream api_backend {
        server api-aponta:8000;
    }

    # Upstream para ds.treit.com.br
    upstream ds_site_backend {
        server treit-design-system-ds-site-1:3000;
    }

    # Upstream para mcp.treit.com.br
    upstream mcp_backend {
        server treit-design-system-mcp-1:8787;
    }

    # Rate limiting
    limit_req_zone $binary_remote_addr zone=api_limit:10m rate=100r/s;

    # Log format
    log_format main '$remote_addr - $remote_user [$time_local] "$request" '
                    '$status $body_bytes_sent "$http_referer" '
                    '"$http_user_agent" "$http_x_forwarded_for"';

    access_log /var/log/nginx/access.log main;
    error_log /var/log/nginx/error.log warn;

    # ========================================
    # api-aponta.pedroct.com.br
    # ========================================

    # HTTP - api-aponta.pedroct.com.br
    server {
        listen 80;
        server_name api-aponta.pedroct.com.br;
        return 301 https://$host$request_uri;
    }

    # HTTPS - api-aponta.pedroct.com.br
    server {
        listen 443 ssl http2;
        server_name api-aponta.pedroct.com.br;

        ssl_certificate /etc/nginx/ssl/fullchain-api.pem;
        ssl_certificate_key /etc/nginx/ssl/privkey-api.pem;

        ssl_protocols TLSv1.2 TLSv1.3;
        ssl_prefer_server_ciphers on;
        ssl_ciphers ECDHE-ECDSA-AES128-GCM-SHA256:ECDHE-RSA-AES128-GCM-SHA256:ECDHE-ECDSA-AES256-GCM-SHA384:ECDHE-RSA-AES256-GCM-SHA384;

        # CloudFlare Real IP headers
        set_real_ip_from 173.245.48.0/20;
        set_real_ip_from 103.21.244.0/22;
        set_real_ip_from 103.22.200.0/22;
        set_real_ip_from 103.31.4.0/22;
        set_real_ip_from 141.101.64.0/18;
        set_real_ip_from 108.162.192.0/18;
        set_real_ip_from 190.93.240.0/20;
        set_real_ip_from 188.114.96.0/20;
        set_real_ip_from 197.234.240.0/22;
        set_real_ip_from 198.41.128.0/17;
        set_real_ip_from 162.158.0.0/15;
        set_real_ip_from 104.16.0.0/13;
        set_real_ip_from 104.24.0.0/14;
        set_real_ip_from 172.64.0.0/13;
        set_real_ip_from 131.0.72.0/22;
        real_ip_header CF-Connecting-IP;

        location / {
            limit_req zone=api_limit burst=20 nodelay;

            proxy_pass http://api_backend;
            proxy_http_version 1.1;

            proxy_set_header Host $host;
            proxy_set_header X-Real-IP $remote_addr;
            proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
            proxy_set_header X-Forwarded-Proto $scheme;
            proxy_set_header CF-Connecting-IP $http_cf_connecting_ip;
            proxy_set_header Connection "";

            proxy_connect_timeout 60s;
            proxy_send_timeout 60s;
            proxy_read_timeout 60s;

            proxy_buffering on;
            proxy_buffer_size 4k;
            proxy_buffers 8 4k;
        }

        location /nginx-health {
            access_log off;
            return 200 "healthy\n";
            add_header Content-Type text/plain;
        }
    }

    # ========================================
    # ds.treit.com.br
    # ========================================

    # HTTP - ds.treit.com.br
    server {
        listen 80;
        server_name ds.treit.com.br;
        return 301 https://$host$request_uri;
    }

    # HTTPS - ds.treit.com.br
    server {
        listen 443 ssl http2;
        server_name ds.treit.com.br;

        ssl_certificate /etc/nginx/ssl/fullchain-ds.pem;
        ssl_certificate_key /etc/nginx/ssl/privkey-ds.pem;

        ssl_protocols TLSv1.2 TLSv1.3;
        ssl_prefer_server_ciphers on;
        ssl_ciphers ECDHE-ECDSA-AES128-GCM-SHA256:ECDHE-RSA-AES128-GCM-SHA256:ECDHE-ECDSA-AES256-GCM-SHA384:ECDHE-RSA-AES256-GCM-SHA384;

        # CloudFlare Real IP headers
        set_real_ip_from 173.245.48.0/20;
        set_real_ip_from 103.21.244.0/22;
        set_real_ip_from 103.22.200.0/22;
        set_real_ip_from 103.31.4.0/22;
        set_real_ip_from 141.101.64.0/18;
        set_real_ip_from 108.162.192.0/18;
        set_real_ip_from 190.93.240.0/20;
        set_real_ip_from 188.114.96.0/20;
        set_real_ip_from 197.234.240.0/22;
        set_real_ip_from 198.41.128.0/17;
        set_real_ip_from 162.158.0.0/15;
        set_real_ip_from 104.16.0.0/13;
        set_real_ip_from 104.24.0.0/14;
        set_real_ip_from 172.64.0.0/13;
        set_real_ip_from 131.0.72.0/22;
        real_ip_header CF-Connecting-IP;

        location / {
            proxy_pass http://ds_site_backend;
            proxy_http_version 1.1;
            proxy_set_header Host $host;
            proxy_set_header X-Real-IP $remote_addr;
            proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
            proxy_set_header X-Forwarded-Proto $scheme;
            proxy_set_header CF-Connecting-IP $http_cf_connecting_ip;
            proxy_set_header Connection "";
        }
    }

    # ========================================
    # mcp.treit.com.br
    # ========================================

    # HTTP - mcp.treit.com.br
    server {
        listen 80;
        server_name mcp.treit.com.br;
        return 301 https://$host$request_uri;
    }

    # HTTPS - mcp.treit.com.br
    server {
        listen 443 ssl http2;
        server_name mcp.treit.com.br;

        ssl_certificate /etc/nginx/ssl/fullchain-mcp.pem;
        ssl_certificate_key /etc/nginx/ssl/privkey-mcp.pem;

        ssl_protocols TLSv1.2 TLSv1.3;
        ssl_prefer_server_ciphers on;
        ssl_ciphers ECDHE-ECDSA-AES128-GCM-SHA256:ECDHE-RSA-AES128-GCM-SHA256:ECDHE-ECDSA-AES256-GCM-SHA384:ECDHE-RSA-AES256-GCM-SHA384;

        # CloudFlare Real IP headers
        set_real_ip_from 173.245.48.0/20;
        set_real_ip_from 103.21.244.0/22;
        set_real_ip_from 103.22.200.0/22;
        set_real_ip_from 103.31.4.0/22;
        set_real_ip_from 141.101.64.0/18;
        set_real_ip_from 108.162.192.0/18;
        set_real_ip_from 190.93.240.0/20;
        set_real_ip_from 188.114.96.0/20;
        set_real_ip_from 197.234.240.0/22;
        set_real_ip_from 198.41.128.0/17;
        set_real_ip_from 162.158.0.0/15;
        set_real_ip_from 104.16.0.0/13;
        set_real_ip_from 104.24.0.0/14;
        set_real_ip_from 172.64.0.0/13;
        set_real_ip_from 131.0.72.0/22;
        real_ip_header CF-Connecting-IP;

        location / {
            proxy_pass http://mcp_backend;
            proxy_http_version 1.1;
            proxy_set_header Host $host;
            proxy_set_header X-Real-IP $remote_addr;
            proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
            proxy_set_header X-Forwarded-Proto $scheme;
            proxy_set_header CF-Connecting-IP $http_cf_connecting_ip;
            proxy_set_header Connection "";
        }
    }
}
EOF
```

## Passo 6: Criar Dockerfile do nginx

```bash
cat > /opt/nginx-ssl/Dockerfile <<'EOF'
FROM nginx:alpine

COPY conf/nginx.conf /etc/nginx/nginx.conf
COPY ssl/*.pem /etc/nginx/ssl/

EXPOSE 80 443

CMD ["nginx", "-g", "daemon off;"]
EOF
```

## Passo 7: Criar network compartilhada (se não existir)

```bash
# Verificar se a network já existe
docker network ls | grep treit-design-system_default

# Se não existir, descobrir o nome correto
docker inspect treit-design-system-ds-site-1 | grep NetworkMode

# Usar o nome da network encontrada, por exemplo:
# Se for "treit-design-system_default", use este nome
# Se for outro, ajuste o comando abaixo

# A network provavelmente já existe, então apenas precisamos conectar o nginx a ela
```

## Passo 8: Build e iniciar o nginx

```bash
cd /opt/nginx-ssl

# Build da imagem
docker build -t nginx-ssl-proxy .

# Parar e remover container antigo se existir
docker stop nginx-ssl-proxy 2>/dev/null || true
docker rm nginx-ssl-proxy 2>/dev/null || true

# Iniciar o container na mesma network dos outros containers
docker run -d \
  --name nginx-ssl-proxy \
  --network treit-design-system_default \
  -p 80:80 \
  -p 443:443 \
  --restart unless-stopped \
  nginx-ssl-proxy
```

## Passo 9: Verificar logs e testar

```bash
# Ver logs do nginx
docker logs -f nginx-ssl-proxy

# Testar configuração
docker exec nginx-ssl-proxy nginx -t

# Ver status dos containers
docker ps | grep -E "(nginx|treit|api)"
```

## Passo 10: Testar os endpoints

```bash
# Testar ds.treit.com.br
curl -I https://ds.treit.com.br

# Testar mcp.treit.com.br
curl -I https://mcp.treit.com.br/health

# Testar api-aponta.pedroct.com.br
curl -I https://api-aponta.pedroct.com.br/health
```

## Troubleshooting

### Se o nginx não conseguir se conectar aos containers:

1. Verificar o nome da network:
```bash
docker network inspect treit-design-system_default
```

2. Verificar os nomes dos containers:
```bash
docker ps --format "{{.Names}}"
```

3. Ajustar os upstreams no nginx.conf com os nomes corretos

### Se aparecer erro de certificado:

1. Verificar se os certificados foram criados:
```bash
ls -la /opt/nginx-ssl/ssl/
```

2. Verificar permissões:
```bash
chmod 644 /opt/nginx-ssl/ssl/*.pem
```

### Para recarregar configuração do nginx:

```bash
docker exec nginx-ssl-proxy nginx -s reload
```

## Próximos passos

1. Gerar certificado SSL para `api-aponta.pedroct.com.br` no Cloudflare
2. Substituir os links simbólicos pelos certificados reais
3. Recarregar o nginx
