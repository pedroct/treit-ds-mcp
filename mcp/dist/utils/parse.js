...
export function extractSummary(source, fallbackName) {
...
export function inferTags(name, filePath, source) {
    const tags = new Set();
    const haystack = `${name} ${filePath} ${source}`.toLowerCase();
    const addIf = (keyword, tag) => {
...
export function extractTailwindColorTokens(tailwindSource) {
    const colors = {};
...
export function extractCssVariables(cssSource) {
    const vars = new Set();
