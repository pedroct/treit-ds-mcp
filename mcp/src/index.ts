...
async function main() {
  ...
  const getResourcePayload = async (uri: string): Promise<unknown | null> => {
  ...
  const handleToolCall = (toolName: string, args: Record<string, unknown>): unknown => {
    if (toolName === "tool.searchComponents") {
      return searchComponents(componentsIndex, args as Parameters<typeof searchComponents>[1]);
    }
